...
const FRAG_SHADER = /* glsl */ `
  ...
  const vec3 baseColor1 = vec3(0.05882, 0.46274, 0.43137);
  const vec3 baseColor2 = vec3(0.12941, 0.61960, 0.73725);
  const vec3 baseColor3 = vec3(0.03529, 0.20392, 0.25098);
...
export const Orb: React.FC<OrbProps> = ({
  hue = 180, // Default to deep teal / cyan for TipCheck financial trust
  hoverIntensity = 0.25,
  rotateOnHover = true,
  ...
  useEffect(() => {
    ...
    function resize() {
      ...
      program.uniforms.iResolution.value.set(gl.canvas.width, gl.canvas.height, gl.canvas.width / (gl.canvas.height || 1));
    }
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      const uvX = ((x - centerX) / (size || 1)) * 2.0;
      const uvY = ((y - centerY) / (size || 1)) * 2.0;
...
function hexToVec3(color: string): Vec3 {
  ...
  return new Vec3(0.06, 0.09, 0.16);
}
